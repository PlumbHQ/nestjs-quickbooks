...
describe('QueryUtils test', () => {
  ...
  it('[2] generateQuery with conditions should return a valid query', () => {
    ...
    expect(querystring.decode(query)).toEqual(
      expect.objectContaining({
        query:
          "select * from customer where Metadata.LastUpdatedAt = '2015-01-01'",
      }),
    );
  });
  ...
  it('[6] generateQuery with conditions should return a valid query', () => {
    ...
    expect(querystring.decode(query)).toEqual(
      expect.objectContaining({
        query:
          "select * from customer where Metadata.LastUpdatedAt = '2015-01-01' and Metadata.CreateAt = '2015-01-01'",
      }),
    );
  });
  ...
  it('[7] generateQuery with conditions should return a valid query', () => {
    ...
    expect(querystring.decode(query)).toEqual(
      expect.objectContaining({
        query:
          "select * from customer where Id >= '0' and Metadata.LastUpdatedAt = '2015-01-01'",
      }),
    );
  });
  ...
  it('[8] generateQuery with conditions should return a valid query', () => {
    ...
    expect(querystring.decode(query)).toEqual(
      expect.objectContaining({
        query:
          "select * from customer where Metadata.LastUpdatedAt = '2015-01-01' and CustomerRef.value = '5'",
      }),
    );
  });
});
